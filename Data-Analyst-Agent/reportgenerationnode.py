...
class ProactiveReportGeneratorPDF:
    ...
    def _call_hf_model(self, prompt: str) -> str:
        """Call Hugging Face model for report summary generation"""
        if not self.client:
            return ""
        try:
    ...
    def generate_pdf_report(self, insights: List[Dict[str, Any]], filename: str = "Proactive_Report.pdf", title: str = "Proactive Data Analysis Report") -> str:
        ...
        summary_prompt = "Summarize the following insights professionally:\n" + "\n".join(
            [f"{i+1}. {insight['answer']}" for i, insight in enumerate(insights)]
        )
    ...
    def run(self, dataset_name: str = None, filename: str = "Proactive_Report.pdf") -> str:
        """
        Entry point for workflow node.
        Generates PDF report from insights stored in state.
        """
        insights = self.state.insights
        if not insights:
            print("⚠️ No insights available to generate report.")
            return ""
        report_file = self.generate_pdf_report(insights, filename)
        return report_file
