...
def data_cleaning_analysis_node(state: ReportState) -> ReportState:
    ...
    for dataset_name, df in processed_tables.items():
        print(f"Cleaning dataset: {dataset_name}")
         ...
         # Detect if dataset looks like unstructured text (has 'content' column)
        if "content" in df.columns and df.shape[1] <= 3:
            ...
            cleaning_report['cleaning_steps'].append("Normalized, tokenized, stopwords removed, lemmatized")

            # Basic analysis for text
            ...
            analysis_results = {
                ...
                'top_words': pd.Series(" ".join(cleaned_df['cleaned_content']).split()).value_counts().head(20).to_dict()
            }
